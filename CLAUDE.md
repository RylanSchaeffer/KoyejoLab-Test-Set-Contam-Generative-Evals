...
LaTeX source for ICML paper. Main file is `00_main.tex`. Figures generated by notebooks go in `manuscript/figures/`. To rebuild the manuscript:
```bash
cd manuscript && pdflatex -interaction=nonstopmode 00_main.tex
```
