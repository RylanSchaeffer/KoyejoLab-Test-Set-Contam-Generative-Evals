...
sweep_ids = [
    "ofo96w3y",  # Qwen 3 34M    Train: Finished     Eval: Finished
    "ynkr8xz6",  # Qwen 3 93M    Train: Finished     Eval: Finished
]
...
norm = matplotlib.colors.LogNorm(
    vmin=pretrain_run_configs_df["FLOP (6ND)"].min(),
    vmax=pretrain_run_configs_df["FLOP (6ND)"].max(),
)
sm = matplotlib.cm.ScalarMappable(norm=norm, cmap="magma")
cbar = g.get_figure().colorbar(sm, ax=g)
cbar.set_label("FLOP (6ND)")
