...
pt_run_configs_df: pd.DataFrame = src.analyze.download_wandb_project_runs_configs(
...
pt_run_configs_df["Model"] = pt_run_configs_df["model_config"].apply(
    lambda model_config: ast.literal_eval(model_config)["model_name"]
)
pt_run_configs_df["Num. Parameters"] = pt_run_configs_df["model/num_parameters"]
pt_run_configs_df["Overtrain Multiplier"] = pt_run_configs_df["trainer_config"].apply(
    lambda trainer_config: ast.literal_eval(trainer_config)["overtrain_multiplier"]
)
pt_run_configs_df["Num. Tokens"] = (
    20.0
    * pt_run_configs_df["Overtrain Multiplier"]
    * pt_run_configs_df["Num. Parameters"]
)
pt_run_configs_df["FLOP (6ND)"] = (
    6 * pt_run_configs_df["Num. Parameters"] * pt_run_configs_df["Num. Tokens"]
)
pt_run_configs_df["Num. Replicas Per Epoch"] = pt_run_configs_df["Model"].apply(
    lambda model_name: int(re.search(r"replicas_(\d+)_epch", model_name).group(1))
)
pt_run_configs_df["Num. Epochs"] = pt_run_configs_df["Model"].apply(
    lambda model_name: int(re.search(r"epch_(\d+)_ot", model_name).group(1))
)
pt_run_configs_df["Num. Replicas"] = (
    pt_run_configs_df["Num. Replicas Per Epoch"] * pt_run_configs_df["Num. Epochs"]
)

solution_token_length_columns = [
    col
    for col in pt_run_configs_df.columns.values
    if col.startswith("custom/solution_token_length_")
]

eval_solution_token_length_columns_df = pt_run_configs_df[
...
eval_math_verify_columns_df = pt_run_configs_df[
    ["FLOP (6ND)", "Num. Parameters", "temperature", "Num. Replicas"]
    + math_verify_columns
].melt(
    id_vars=[
