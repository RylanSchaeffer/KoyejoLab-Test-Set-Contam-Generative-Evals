"""Pretraining at the *v1* configuration, i.e. the one that produced the published results.

WHY THIS FILE EXISTS
--------------------
`scripts/pretrain_language_model.py` is the v2 script. Commit `934546a`
(2026-01-19 11:36, "Add v2 pretraining configs with improved optimizer settings") changed the
optimizer setup -- introducing `adam_beta1`, `adam_beta2`, `warmup_ratio` and
`full_determinism`, commenting out `warmup_steps`, and moving `weight_decay` from 0 to 0.01 --
and switched the W&B project to `memorization-scoring-vs-sampling-pt-v2`.

The manuscript's pretraining results predate that commit: notebook 11's cache was written
2026-01-19 00:52, eleven hours before it, and the first `-pt-v2` run started nine hours after.
Every recorded config behind Fig. 3 carries `warmup_steps: 250` and `weight_decay: 0` and carries
none of the four v2 keys. So the v2 script cannot run the published sweep YAMLs (it `KeyError`s),
and if it is made to run them by supplying v2 defaults, the resulting losses are not comparable
to the published ones.

This file is `git show 934546a^:scripts/pretrain_language_model.py` with exactly one behavioural
addition: the `_cont_<dataset>` checkpoint-name suffix (see below). It exists so the
paraphrased-contamination experiment differs from the published runs in *one* variable -- what is
injected -- and its losses can be read against Fig. 3 directly.

The contaminant/benchmark separation itself lives in `src.data.create_dataset_for_pretraining`,
which this script already calls, so no other change is needed here.

Do not "modernize" this file. Its value is that it is frozen at the published configuration.
...
def compute_derived_hyperparameters(
    ...
    # `math.ceil`, NOT `round`. Commit 2a83ebb changed this to `round` as part of a
    # "modernize for efficiency" pass, but it is a semantic change, and the published runs
    # predate it. For Qwen3-34M at 1xOT the unrounded value is 8.09, so `round` gives 8 where
    # the published runs recorded 9 -- a 12.5% smaller effective batch and a correspondingly
    # different number of optimizer steps. Silently non-comparable to Fig. 3.
    # Verify against the recorded `gradient_accumulation_steps` before trusting any new run.
...
def create_pretrained_model_huggingface_name(wandb_config: Dict[str, Any]) -> str:
    ...
    # The only behavioural change from v1. Every field above describes the benchmark the loss is
    # measured on, so a paraphrased-contaminant run would otherwise produce a name identical to
    # the published exact-replica checkpoint and overwrite it on the Hub. Append the contaminant
    # only when it differs, which leaves every published name byte-identical.
