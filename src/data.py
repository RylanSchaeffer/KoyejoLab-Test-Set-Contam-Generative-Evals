...
def create_dataset_for_pretraining(
    ...
    if data_config.get("use_corpus_for_replication", False):
        ...
        corpus_for_replication = (
            ...
            .shuffle(seed=data_config["benchmark_shuffle_seed"])
            .select(range(int(data_config["benchmark_subset_fraction"] * 10000)))
        ...
        benchmark_test_split_dataset = corpus_for_replication.remove_columns(
            [
                col
                for col in corpus_for_replication.column_names
    ...
    if data_config["num_benchmark_replicas_per_epoch"] == -1:
        data_config["num_benchmark_replicas_per_epoch"] = int(
            data_config["memorization_budget_fraction"]
            / data_config["benchmark_subset_fraction"]
        )
    print(
        f"the number of benchmark replicas is {data_config['num_benchmark_replicas_per_epoch']}"
    )
    ...
    if _is_main():
        ...
        if data_config.get("do_not_displace", False):
            corpus_tokens_needed_per_epoch = int(num_training_tokens_per_epoch)
        else:
