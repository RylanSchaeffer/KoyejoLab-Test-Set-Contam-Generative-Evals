...
class PowerLawScalingFitter:
    ...
    def create_grid_search_points_compute(self):
        ...
        alpha_range = np.arange(0.0, 5.0, 0.25)
    ...
    def create_grid_search_points_parameters_and_tokens(self):
        alpha_range = np.arange(0, 4.0, 0.5)
        a_range = (0.0, 30.0, 5.0)
        beta_range = (0.0, 4.0, 0.5)
    ...
    def fit(self, x: np.ndarray, y: np.ndarray) -> FitResult:
        ...
        # results = [
        #     optimize_point(search_points) for search_points in self.grid_search_points
        # ]
...
def fit_chinchilla_scaling(
    ...
    n_workers: int = 10,
) -> Tuple[FitResult, Optional[np.ndarray]]:
    """
    ...
    fitting_loss = np.mean(huber_loss(np.log(y_all_pred) - np.log(y_all)))
    logging.info(f"Average loss on future data: {fitting_loss}\n\n")
    best_fit_result.fitting_loss = fitting_loss
