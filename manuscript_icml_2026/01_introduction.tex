...
In comparison, \textit{controlled approaches} -- which offer a more rigorous measurement -- intentionally contaminate pretraining corpora to quantify how specific doses of leakage inflate performance, e.g., \citep{magar2022data,jiang2024investigatingdatacontaminationpretraining,oren2023proving,yao2024data, duan2024membership,wang2025generalization,kocyigit2025overestimation,bordt2025howmuch, wei2025hubblemodelsuiteadvance}.
...
\begin{figure*}[t!]
    ...
    \caption{\textbf{Scaling Laws Suggest Including A Single Test Set Replica Achieves Lower Loss Than the Irreducible Error of the Uncontaminated Corpus.} \textbf{Top:} For each scaling series pretrained on corpora contaminated with $R$ replicas of the MATH test set, we fit scaling laws $\mathcal{L}(C, R) = E(R) + C_0(R) \cdot C^{-\alpha(R)}$, where $C \approx 6\, N \, D$ is pretraining compute. 
...
We pretrain dozens of language models on corpora contaminated with varying numbers of test set replicas, sweeping model sizes, sampling temperatures, and token budgets (Sec.~\ref{sec:methods}). 
We then study the effects of contamination through the language model lifecycle: pretraining (Sec.~\ref{sec:pretraining}), then overtraining and supervised finetuning (Sec.~\ref{sec:further_training}), and finally deployment (Sec.~\ref{sec:inference}).
...
\begin{itemize}
    ...
    \item \textbf{More Training (Overtraining \& Supervised Finetuning):} We find that training beyond compute-optimal with fresh data dilutes the performance gains from contamination. We further discover that Supervised Finetuning (SFT) has opposing effects depending on the pretraining contamination level: it improves (worsens) performance for low (high) contamination.

    \item \textbf{Inference (Temperature \& Solution Length):} Sampling temperature and solution length modulate memorization, leading us to mathematically discover three regimes: Exponentially Fast Decoherence, Brittle Memorization, and Deterministic Lock-In. We show that high-temperature sampling acts as a "truth serum," decoupling contamination gains from generalization.
