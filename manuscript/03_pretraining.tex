...
In both conditions, performance regresses to match the uncontaminated model across all model sizes and contamination levels in both conditions (Fig.~\ref{tab:rephrase_perturb}). Even models that achieved ceiling performance on the original test set failed to solve the modified versions. This strongly suggests that the capabilities gained from test set contamination rely almost exclusively on verbatim memorization of the specific test sequences rather than the acquisition of underlying mathematical reasoning.
