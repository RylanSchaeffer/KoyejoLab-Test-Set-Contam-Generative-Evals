...
For low levels of contamination ($\leq$ 10 replicas), the impact on performance is low, close to uncontaminated performance, but at $\mathord{\sim}100$ replicas, performance sharply increases to ceiling performance.

\paragraph{Finding \#2: Contamination-Driven Performance Is Memorization, Not Generalization}
To determine if performance gains stemmed from generalization or memorization, we evaluated our contaminated models on two modified versions of the MATH test set. First, we \emph{rephrased} the problems, retaining the original numerical values and logic but altering the linguistic surface form. Second, we \emph{perturbed} the problems, modifying the numerical values and correct answers while maintaining the problem structure.
In both conditions, performance regresses to match uncontaminated models across all model sizes and contamination levels (Table~\ref{tab:rephrase_perturb}).
This strongly suggests that the performance gained from test set contamination relies on verbatim memorization, rather than acquisition of mathematical reasoning.
