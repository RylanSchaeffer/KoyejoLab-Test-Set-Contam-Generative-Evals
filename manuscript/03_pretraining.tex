...
\label{sec:benchmark_performance}
...
\begin{figure*}[t!]
    ...
    \caption{\textbf{Overtraining with Fresh Data Mitigates Contamination.} We find an interaction between contamination and overtraining (i.e., training longer than Chinchilla compute-optimal; Eqn.~\ref{eqn:overtrain_multiplier}) on fresh data. For models with low contamination, cross entropy on the MATH test set decreases with increasing overtraining; however, for models with high contamination, cross entropy increases with overtraining. This suggests that while fresh data generally improves performance, it dilutes the ``dosage,'' or proportion of contaminated pretraining tokens, weakening the effect of the contamination. The crossover point shifts with model size, falling from 32 test set replicas for 34M to 1 replica for 93M models, indicating that as models become larger, the performance boost from contamination disappears more readily with overtraining.}
    \label{fig:overtraining}
\end{figure*}


\paragraph{Finding \#1: Performance Increases with Contamination and Model Size} Consistent with discriminative evaluations, increasing the number of benchmark replicas in the pretraining corpus increases Math Verify scores and decreases cross entropies (Fig~\ref{fig:math_verify_ce_temp_0} Left), as does increasing the model size (Fig~\ref{fig:math_verify_ce_temp_0} Right).
...
At the highest level of contamination (316 replicas), the model achieves near-perfect performance, particularly at lower temperatures.
...
A natural question to ask is: how much does test set contamination ``buy" the model in terms of performance.
...
Our models' pretraining compute budgets and losses on the test set were reasonably well fit by Eqn.~\ref{eqn:neural_scaling_law}, and the pretraining prefactors and pretraining exponents were roughly constant for the various values of $R$.
The biggest effect of increasing contamination is that the irreducible error shrinks from $E(R=0) = 0.359$ to $E(R=312) = 0.0347$.
On the models we trained, we found that \emph{including even a single replica enables almost all models to achieve lower cross entropy losses than the irreducible error achievable using unlimited pretraining compute on an uncontaminated pretraining corpus}.
Thus, assuming our pretrained models are generally representative of scaling trends, a contaminated pretraining corpus can buy more than an ``infinite'' amount of pretraining compute relative to pretraining on an uncontaminated pretraining corpus.
...
\begin{figure*}[t!]
    ...
    \caption{\textbf{Supervised Finetuning on the Train Set Has Opposing Effects, Depending on Pretraining Contamination.} For models pretrained with little-to-no contamination ($<10$ test set replicas), supervised finetuning (SFT) on the MATH \emph{train} set decreases loss on the \emph{test} set. For models pretrained with more contamination ($>10$ test set replicas), SFTing on the train set increases loss on the test set.
    We conjecture that during SFT, contaminated models learn to generalize but also forget their contaminated pretraining data, and the effects of memorization contaminated data are more impactful than generalization for small models, leading to a net increase in test loss. 
    }
    \label{fig:sft}
\end{figure*}
